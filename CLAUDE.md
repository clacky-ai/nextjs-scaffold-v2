...
- **User System**: Routes at `/`, auth at `/api/auth`, store in `localStorage['auth-storage']`
- **Admin System**: Routes at `/admin`, auth at `/api/admin/auth`, store in `localStorage['admin-auth-storage']`
...
- **API Routes**:
  ...
  - Admin APIs: `/api/admin/` (admin authentication required)

### Database Schema
- **users**: Regular user accounts (id, email, password, realName, phone, organization, etc.)
- **adminUsers**: Admin accounts (id, username, password, name, email)
- Both tables use varchar IDs and include timestamps
