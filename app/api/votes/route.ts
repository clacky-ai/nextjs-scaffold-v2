...
export async function POST(request: NextRequest) {
  try {
    ...
    if (teamMembers.includes(session.user.id) || project.submitterId === session.user.id) {
      return NextResponse.json(
        { error: '不能给自己的项目投票' },
        { status: 403 }
      )
    ...
    if (userVoteCount[0].count >= (systemStatus.maxVotesPerUser || 3)) {
      return NextResponse.json(
        { error: `您已达到最大投票数量限制（${systemStatus.maxVotesPerUser || 3}票）` },
        { status: 400 }
      )
    }
