...
interface UserWebSocketStore {
  ...
  onSystemStatusUpdate: (
    callback: (data: SystemStatusUpdateData) => void
  ) => void;
...
export const useUserWebSocketStore = create<UserWebSocketStore>((set, get) => {
  ...
  let systemStatusCallback: ((data: SystemStatusUpdateData) => void) | null =
    null;
  ...
  return {
    ...
    setupStoreIntegrations: () => {
      // This method will be called by the store provider to set up
      // integrations with other stores for real-time updates
      console.log("WebSocket store integrations set up");
    },
  };
});
